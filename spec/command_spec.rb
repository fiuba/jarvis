...
describe Command do
	...
	describe 'execute' do
		...
		it 'should return true when system call returns 0' do
			script_path = File.expand_path(File.dirname(__FILE__) + '/resources/script_ok.sh')
		...
		it 'should return false when system call returns 1' do
			script_path = File.expand_path(File.dirname(__FILE__) + '/resources/script_fail.sh')
			cmd = Command.with_statement("bash #{script_path}")
			cmd.execute.should be false
		end

		it 'should capture output' do
			script_path = File.expand_path(File.dirname(__FILE__) + '/resources/script_some_output.sh')
			cmd = Command.with_statement("bash #{script_path}")
			cmd.execute
